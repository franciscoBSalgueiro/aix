...
inline void FenAtPositionFromFen(DataChunk &args, ExpressionState &state, Vector &result) {
	TernaryExecutor::ExecuteWithNulls<string_t, int32_t, string_t, string_t>(
	    args.data[0], args.data[1], args.data[2], result, args.size(),
	    [&](string_t game, int32_t pos, string_t initial_fen, ValidityMask &mask, idx_t idx) {
		    diplomat::span<const uint8_t> data = {const_data_ptr_cast(game.GetData()), game.GetSize()};
		    auto fen_result = Game::fen_at_position_from_fen(
		        data, pos, std::string_view(initial_fen.GetData(), initial_fen.GetSize()));
		    auto fen_opt = UnwrapOptionalDecoded<std::string>(std::move(fen_result), "fen_at_position");
